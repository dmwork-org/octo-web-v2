...
| --- | --- | --- | --- |
| `/v1/friend/sync` | POST | `features/contacts/api/friends.api.ts` | 全量 / 增量同步好友列表 |
| `/v1/friend/search` | GET | 同上 | 搜索好友(给加好友页用) |
| `/v1/friends/:uid` | DELETE | 同上 | 删除好友 |
| `/v1/friend/remark` | PUT | 同上 | 设置备注 |
| `/v1/friend/apply` | POST | `features/contacts/api/friend-applies.api.ts` | 发起好友申请 |
| `/v1/friend/apply` | GET | 同上 | 查询好友申请列表(可能用 `/friend/applies`,commit 0 校对) |
| `/v1/friend/apply/:id` | DELETE | 同上 | 删除申请记录 |
| `/v1/friend/sure` | POST | 同上 | 接受申请 |
| `/v1/user/reddot/friendApply` | POST | 同上 | 清空新朋友红点 |

**校验任务**(commit 0 audit):

- 逐条对照旧 `dmworkcontacts/src/NewFriend/vm.tsx` / `FriendAdd/` 的 `WKApp.apiClient.get/post` 调用,确认 path / method / body / response shape 一致
- response 类型是否完整(IC 自查 Friend / FriendApply types 字段够不够)
- 重复 endpoint 是否合并(例如 `GET /friend/apply` 是否就是申请列表)
- 缺口 endpoint 是否需要本期补(audit 报告标 ⚠️ 项目)
