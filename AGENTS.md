...
- **类型检查**:`vp build`(Vite)不做类型检查。提交前务必 `npx tsc -b` 确认零错误,避免未使用的 import/变量(TS6133)、访问不存在的属性(TS2339)、类型不兼容(TS2345)等问题
...
- [ ] Run `npx tsc -b` to type-check — local `vp build` skips TypeScript project checks.
