...
## P2-C(W5 末)— Chat 视觉 1:1 对齐(此前 P1-B 只做了 sidebar 视觉,chat 内容区遗漏)

> 当前 chat 是 tailwind + token 临时样式,与旧项目 Figma 设计稿差距大。
> 本阶段对照 `packages/dmworkbase/src/{Messages,Components/Conversation,Components/ConversationList,Components/MessageInput}/*.css` 逐项还原。
> 不引新功能,纯视觉 + 微交互(hover/active/动画)。

| ID    | 任务                                              | 旧项目对照                                                            | 验收                                                               |
| ----- | ------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------ |
| P2-C1 | 会话列表行视觉 1:1                                | `Components/ConversationList/index.css` `.wk-conversation-item-*`     | 行高/头像 40px/数字徽标位置/digest 单行截断/时间格式 mm-dd / HH:mm |
| P2-C2 | 消息气泡视觉 1:1(Text/Image/File)                 | `Messages/Base/index.css` `.wk-message-row-*` + 各 renderer index.css | 圆角/max-w 70% / 双色(self/other) / sender 名字 / timestamp 悬浮   |
| P2-C3 | Composer 视觉 1:1                                 | `Components/MessageInput/index.css`                                   | 工具栏 icon 间距/输入框最小高/分割线/发送按钮 brand 色             |
| P2-C4 | Chat header 视觉 1:1                              | `Components/Conversation/index.css` `.wk-conversation-header-*`       | 高度 56px / 头像 + title + 在线点 / 右侧 search/设置占位           |
| P2-C5 | 状态徽标 / 撤回提示 / 系统消息视觉 1:1            | `Messages/{Revoke,System,Time,HistorySplit}/index.css`                | 居中灰条/字号/颜色与旧项目像素级一致                               |
| P2-C6 | 时间分组(同分钟合并 / 跨日 historySplit 自动插入) | `Conversation/vm.ts::insertHistorySplit` + Time renderer              | 时间标签 + 分割线规则与旧一致                                      |
| P2-C7 | hover / active / 选中 / 焦点视觉                  | 全局微交互 transition 200ms ease-(--ease-emphasized)                  | 与旧项目 hover 颜色 / 动画曲线一致                                 |
| P2-C8 | 右键菜单 / 长按反馈占位                           | UI 留位,真实菜单 P3-C8 接                                             | 长按 message 出 ContextMenu placeholder(P3 替换)                   |
...
- **再进 P2-C**(chat 视觉 1:1 对齐)— 与旧项目像素级还原,纯样式微交互
