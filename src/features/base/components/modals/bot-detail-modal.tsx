...
import { BotManageModal } from "@/features/chat/components/bot-manage-modal";
 ...
 * - 创建者 / 命令(若有)
 * - **Bot 管理入口**(对齐上游 e7c5e0be / #235):owner-only 一行 entry button,
 *   点开 BotManageModal(三级下钻:菜单 → 免@回答群列表)
...
export function BotDetailModal({ uid, onClose }: BotDetailModalProps) {
  ...
  const [showBotManage, setShowBotManage] = useState(false);

  // 切 bot(uid 变)时关掉子 modal,避免上一个 bot 的 BotManage 串台
  useCloseBotManageOnUidChange(uid, setShowBotManage);
    ...
    <>
      ...
      {uid && isOwner ? (
        <BotManageModal
          open={showBotManage}
          robotId={uid}
          onClose={() => setShowBotManage(false)}
        />
      ) : null}
    </>
  );
...
/**
 * 切 uid 时关掉 BotManage 子 modal,避免上一个 bot 的 BotManage 串台
 * (no-useeffect-in-component 规则要求抽到命名 hook)。
 */
function useCloseBotManageOnUidChange(
  uid: string | null,
  setShowBotManage: (next: boolean) => void,
): void {
  useEffect(() => {
    setShowBotManage(false);
    // setShowBotManage 是稳定 setter,不入依赖
