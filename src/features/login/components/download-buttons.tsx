...
export function AndroidDownloadButton() {
  ...
  return (
    <a
      href={apkUrl}
      download
      className="inline-flex cursor-pointer items-center justify-center gap-1.5 rounded-full border-[1.5px] border-[#1C1C23] px-[18px] py-2 text-[13px] text-[#1C1C23] no-underline transition-opacity hover:opacity-80"
    >
      ...
      <span>{t("login.download.android")}</span>
    </a>
  );
}
...
export function IosDownloadButton() {
  ...
  return (
    ...
    >
      ...
      <span>{t("login.download.ios")}</span>
    </a>
  );
}
