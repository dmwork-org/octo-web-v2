...
export function JoinSpaceModal({ open, onClose }: JoinSpaceModalProps) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setInlineError(null);
    if (!codeValid) return setInlineError(t("space.join.invalidCode"));
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        <header className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-text-primary">{t("space.join.title")}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <label className="block text-sm text-text-secondary">
          {t("space.join.codeLabel")}
          <input
            type="text"
            value={code}
            onChange={(e) => setCode(e.target.value.trim())}
            placeholder={t("space.join.codePlaceholder")}
        ...
        {info ? (
          <div className="rounded-md bg-brand-tint px-3 py-2 text-xs text-text-primary">
            {t("space.join.willJoin")} <strong>{info.space_name}</strong>
        ...
        <div className="flex justify-end gap-2">
          <Button onClick={onClose}>{t("space.join.cancel")}</Button>
          <Button
            htmlType="submit"
            type="primary"
            theme="solid"
